...
class FoundationPose:
  def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, scorer:ScorePredictor=None, refiner:PoseRefinePredictor=None, glctx=None, debug=0, debug_dir='/home/bowen/debug/novel_pose_debug/'):
    ...
    self.last_poses = None  # Used for tracking; per the centered mesh
  ...
  def register(self, K, rgb, depth, ob_masks, ob_id=None, glctx=None, iteration=5):
    ...
    best_poses = []
    last_poses = []
    for i, ob_mask in enumerate(ob_masks):
      normal_map = None
      ...
      best_poses.append(poses[0]@self.get_tf_to_centered_mesh())
      best_ids.append(ids[0])
      last_poses.append(poses[0])
  
    self.last_poses = torch.stack(last_poses, axis=0)
    self.pose_last = self.last_poses[0]
    self.best_ids = torch.tensor(best_ids)

    return torch.stack(best_poses, axis=0).data.cpu().numpy()
  ...
  def track_one(self, rgb, depth, K, iteration, extra={}):
    ...
    # new_poses = torch.zeros_like(self.last_poses).to("cuda")
    # for i, last_pose in enumerate(self.last_poses):
    #   pose, vis = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth.cpu().numpy(), K=K, ob_in_cams=last_pose[None].cpu().numpy(), normal_map=None, xyz_map=xyz_map, mesh_diameter=self.diameter, glctx=self.glctx, iteration=iteration, get_vis=self.debug>=2)
    #   new_poses[i] = pose[0]
    # self.last_poses = new_poses
    # return(
    #   torch.einsum('...ij, ...jk -> ...ik', new_poses, self.get_tf_to_centered_mesh())
    # ).data.cpu().numpy().reshape(-1, 4,4)

    poses, _ = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=self.last_poses.data.cpu().numpy(), normal_map=None, xyz_map=xyz_map, mesh_diameter=self.diameter, glctx=self.glctx, iteration=iteration, get_vis=self.debug>=2)
    self.last_poses = poses
    return(
      torch.einsum('...ij, ...jk -> ...ik', poses, self.get_tf_to_centered_mesh())
    ).data.cpu().numpy().reshape(-1, 4,4)
